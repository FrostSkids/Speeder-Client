...
public class GuiOptions extends GuiScreen implements GuiYesNoCallback
{
    ...
    public void initGui() {
        ...
        for (int length = (field_146440_f = GuiOptions.field_146440_f).length, j = 0; j < length; ++j) {
            ...
            if (gamesettings$options.getEnumFloat()) {
                ...
                final GuiOptionButton guioptionbutton = new GuiOptionButton(gamesettings$options.returnEnumOrdinal(), this.width / 2 - 155 + i % 2 * 160, this.height / 6 - 12 + 24 * (i >> 1), gamesettings$options, this.game_settings_1.getKeyBinding(gamesettings$options));
        ...
        if (this.mc.theWorld != null) {
            ...
            final GuiOptionButton guioptionbutton2 = new GuiOptionButton(GameSettings.Options.REALMS_NOTIFICATIONS.returnEnumOrdinal(), this.width / 2 - 155 + i % 2 * 160, this.height / 6 - 12 + 24 * (i >> 1), GameSettings.Options.REALMS_NOTIFICATIONS, this.game_settings_1.getKeyBinding(GameSettings.Options.REALMS_NOTIFICATIONS));
            this.buttonList.add(guioptionbutton2);
        }
    ...
    protected void actionPerformed(final GuiButton button) throws IOException {
        if (button.enabled) {
            ...
            if (button.id == 107) {
                this.mc.gameSettings.saveOptions();
                final IStream istream = this.mc.getTwitchStream();
