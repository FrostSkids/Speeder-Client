...
import Benz.ServerDataFeatured;
...
public class ServerListEntryNormal implements GuiListExtended.IGuiListEntry
{
            ...
            ServerListEntryNormal.field_148302_b.submit(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        ServerListEntryNormal.this.field_148301_e.serverMOTD = EnumChatFormatting.DARK_PURPLE + "Sin Client Cant Resolve Host Name Try Typing IP Again.";
                    }
                    catch (Exception var3) {
                        ServerListEntryNormal.this.field_148301_e.pingToServer = -1L;
                        ServerListEntryNormal.this.field_148301_e.serverMOTD = EnumChatFormatting.DARK_BLUE + "Can't connect to server.";
                    }
                }
            });
        }
        final boolean isFeaturedServer = this.field_148301_e instanceof ServerDataFeatured;
        if (isFeaturedServer) {
            this.drawimg(x, y, false, ServerDataFeatured.SIN_ICON);
        }
    ...
    private void drawimg(final int x, final int y, final boolean lower, final ResourceLocation texture) {
        this.mc.getTextureManager().bindTexture(texture);
        Gui.drawModalRectWithCustomSizedTexture(x - 24, lower ? (y + 24) : y, 0.0f, 0.0f, 24, 24, 24.0f, 24.0f);
    }
