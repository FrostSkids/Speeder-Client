...
public class NetHandlerPlayServer implements INetHandlerPlayServer, ITickable
{
        ...
        if (this.func_183006_b(packetIn)) {
            ...
            if (!this.playerEntity.playerConqueredTheEnd) {
                ...
                if (this.hasMoved) {
                    ...
                    if (d19 - d18 > 100.0 && (!this.serverController.isSinglePlayer() || !this.serverController.getServerOwner().equals(this.playerEntity.getCommandSenderName()))) {
                        NetHandlerPlayServer.logger.warn(String.valueOf(this.playerEntity.getCommandSenderName()) + " moved too quickly! " + d15 + "," + d16 + "," + d17 + " (" + d15 + ", " + d16 + ", " + d17 + ")");
                    ...
                    if (d19 > 0.0625 && !this.playerEntity.isPlayerSleeping() && !this.playerEntity.theItemInWorldManager.isCreative()) {
                        flag2 = true;
                        NetHandlerPlayServer.logger.warn(String.valueOf(this.playerEntity.getCommandSenderName()) + " moved wrongly!");
                    }
                    ...
                    if (!this.serverController.isFlightAllowed() && !this.playerEntity.capabilities.allowFlying && !worldserver.checkBlockCollision(axisalignedbb)) {
                        if (d16 >= -0.03125) {
                            ...
                            if (this.floatingTickCount > 80) {
                                NetHandlerPlayServer.logger.warn(String.valueOf(this.playerEntity.getCommandSenderName()) + " was kicked for floating too long!");
    ...
    public void onDisconnect(final IChatComponent reason) {
        NetHandlerPlayServer.logger.info(String.valueOf(this.playerEntity.getCommandSenderName()) + " lost connection: " + reason);
    ...
    public void processHeldItemChange(final C09PacketHeldItemChange packetIn) {
        ...
        if (packetIn.getSlotId() >= 0 && packetIn.getSlotId() < InventoryPlayer.getHotbarSize()) {
            ...
            NetHandlerPlayServer.logger.warn(String.valueOf(this.playerEntity.getCommandSenderName()) + " tried to set an invalid carried item");
        }
    }
    ...
    public void processVanilla250Packet(final C17PacketCustomPayload packetIn) {
        ...
        if ("MC|BEdit".equals(packetIn.getChannelName())) {
            ...
            try {
                ...
                NetHandlerPlayServer.logger.error("Couldn't handle book info", (Throwable)exception3);
                return;
            }
            finally {
            ...
            try {
                ...
                if (itemstack3 != null) {
                    ...
                    if (itemstack3.getItem() == Items.written_book && itemstack4.getItem() == Items.writable_book) {
                        itemstack4.setTagInfo("author", new NBTTagString(this.playerEntity.getCommandSenderName()));
                ...
                NetHandlerPlayServer.logger.error("Couldn't sign book", (Throwable)exception4);
                return;
            }
            finally {
        ...
        if ("MC|TrSel".equals(packetIn.getChannelName())) {
            try {
                ...
                NetHandlerPlayServer.logger.error("Couldn't select trade", (Throwable)exception5);
            }
        }
            ...
            if (!this.serverController.isCommandBlockEnabled()) {
                ...
                try {
                    ...
                    NetHandlerPlayServer.logger.error("Couldn't set command block", (Throwable)exception6);
                    return;
                }
                finally {
            ...
            if (this.playerEntity.openContainer instanceof ContainerBeacon) {
                try {
                    ...
                    NetHandlerPlayServer.logger.error("Couldn't set beacon", (Throwable)exception7);
                }
            }
        }
